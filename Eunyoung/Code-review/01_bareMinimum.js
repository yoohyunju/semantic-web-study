...
_.drop = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  //왜 여기서는 조건을 적지 않아도 바로 통과되는가? -> slice에서 온다
...
_.last = function (arr, n) {
  ...
  return _.drop(arr, arr.length - n); //!! 짱이다
...
_.uniq = function (arr) {
  ...
  let result = [];
  _.each(arr, function(el) {
    //each는 이미 반복해서 요소를 불러오므로 for문이 따로 필요없음
...
_.pluck = function (arr, keyOrIdx) {
  ...
  return _.map(arr, function(item) {
    return item[keyOrIdx]; //따로 result 값을 주지 않고도 그 값 자체를 return 하는 중이다.
    // 특히 여기서는 map 함수를 바로 사용해 그 값을 주어진 조건에 맞춰 리턴하고 있다.
  })
  */
};
...
// 이때, acc는 '이전 요소까지'의 반복 작업의 결과로 누적된 값입니다. --> acc는 누적된 값으로
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  _.each(arr, function(ele, idx) {
    //초기값이 없고, 들어온 값의 인덱스가 0일떄 배열의 첫번째 인자를 누적값으로 넣어준다.
    if(initVal === undefined && idx === 0) {
      acc = ele;
    }
    //초기값이 존재하면, acc(누적값)에 연산한 값을 할당(저장)해준다.
    else {

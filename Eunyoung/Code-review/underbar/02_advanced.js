...
_.once = function (func) {
  ...
  return function () {
    ...
    //호출 된 적이 없다면 func를 실행 -> call을 통해 함수 실행 유무를 파악
    if(!call) {
      call = true;
    //함수가 한번 실행된 이후에는 그 값이 그대로 나와야하므로 그 값을 저장해준다.
...
// 예를 들어, _.delay(func, 500, 'a', 'b')의 결과로 '최소' 500ms가 지난 이후에 func('a', 'b')가 호출됩니다.
// 필요하면, Chapter - 비동기를 예습하세요.
_.delay = function (func, wait) {
  // TODO: 여기에 코드를 작성합니다.
  // ...arguments = func, 500, a, b
  let arg = [...arguments].slice(2); // [a, b]
  setTimeout(function () { //? 정해젼 형식인건가 -> setTimeout(func(..arg), wait) -> 실행X
    func(...arg) // a, b
  }, wait);
};
...
//
  ...
  //});
  //return result;
  //??????????? reduce를 사용해서는 어떻게 할까? 왜 함수 이름만 바꿨는데 가능할까? 이해X
  //아래의 코드는 reduce를 사용하지만, 그 기능을 사용하지 않고 있다
  let result = false;
  _.reduce(arr, function(el) {
    if(el === target) {
      result = true; //result의 값만을 변경(재할당)하고 그 값을 리턴하고 있기 때문에 
    }
  });
  return result; //reduce 함수를 온전히 이용했다고 보기 어렵다.
};
...
_.every = function (arr, iteratee) {
  ...
  for(let i of arr) {
    if(iteratee === undefined) {
      //모든 요소가 true일 때 true를 반환하므로 거짓일 경우만 false를 반환해준다.
      if(!i) return false;
    }
    else {
      //모든 요소의 통과해야하므로 경우의 수가 작은 false값을 기준으로 리턴해준다.
      if(!iteratee(i)) return false;
    }
  }
  return true;
};

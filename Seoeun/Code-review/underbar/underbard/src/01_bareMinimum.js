...
_.drop = function (arr, n) {
  if (n === undefined || n < 0) return arr; // 조건이 슬라이스에 이미 있어서 굳이 안써도됨
...
_.last = function (arr, n) {
  if (n === undefined || n < 0) return _.drop(arr, arr.length - 1);
...
_.each = function (collection, iteratee) {
  if (Array.isArray(collection)) {
    ...
    // {a:'abxd', b:'dddd'}
    for (let i in collection) iteratee(collection[i], i, collection);
  }
};
...
_.filter = function (arr, test) {
  const result = [];
  _.each(arr, (i) => {
    test(i) && result.push(i); // 처음께 truty한 값이면 뒤에껄 통화 if(test(i)) result.push(i);
  });
  return result;
};
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  if (initVal === undefined) {
    ...
    _.each(newArr, (i, index) => {
      acc = iteratee(acc, i, index, newArr);
    });
  } else {
    acc = initVal;
    _.each(arr, (i, index) => {
      acc = iteratee(acc, i, index, arr);
    });
  } // side effect 가 터질수도? 반복문을 한번더 한거니까
  // let acc = initVal;
  // _.each(arr, (i, idx) => {
  //   if (initVal === undefined && idx === 0) acc = i;
  //   else acc = iteratee(acc, i, idx, arr);
  // });
  return acc;
};

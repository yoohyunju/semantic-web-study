...
_.includes = function (arr, target) {
  ...
  let res = false;
  _.each(arr, (i) => {
    if (i === target) res = true; // return 하면 안됨
  });
  return res;
};
// el 자체가 초기값인거고 reduce의 기능을 못하고 단순 반복문만 하는것 이여서 reduce로 바꿔도 할수는 있는데 굳이 필요없다.
...
_.extend = function (obj1, ...objs) {
  // objs = [obj1, obj2...]
  // 반복문 돌면서 첫 obj1에 계속 추가
  // obj1 에 동일한 키 값있으면 값만 덮어씌움
  // return obj1
  if (Object.keys(obj1).length === 0 && Object.keys(objs[0]).length === 0)
    return obj1; // 매개변수가 다 비어있을때
  _.each(objs, (i) => {
    obj1 = {
      ...obj1,
      ...i,
...
_.defaults = function (obj, ...args) {
  ...
  for (let arg of args) {
    for (let i in arg) {
      obj.hasOwnProperty(i) ? obj : (obj[i] = arg[i]);
    }
  }
  return obj;
};

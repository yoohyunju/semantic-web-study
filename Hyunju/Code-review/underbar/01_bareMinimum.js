...
_.take = function (arr, n) {
  ...
  if(n > arr.length) return result = arr;
...
_.last = function (arr, n) {
  ...
  // arr = [1, 2, 3, 4] // arr.last[2] -> [3, 4] 2 3 index 
  let result = [];
...
_.each = function (collection, iteratee) { // iteratee: 반복을 당하는 함수
  // TODO: 여기에 코드를  작성합니다.
  
  // collection이 배열인 경우
  if(Array.isArray(collection)) {
    // 배열을 순회하면서 배열 요소, 인덱스, 배열 자체에 접근
    for(let i = 0; i < collection.length; i++) {
      iteratee(collection[i], i, collection);
    }
  } else { // collection이 객체인 경우
    // 객체를 순회하면서 속성값, 속성명, 객체 자체에 접근
...
_.indexOf = function (arr, target) {
  ...
  _.each(arr, function (item, index) {
    if (item === target && result === -1) { // target 값 중복 시 이전에 만난 target의 값으로 result가 이미 변경되었기 때문에 if문 실행 안함
      result = index;
    }
  });
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  _.each(arr, (ele, idx, arr) => {
    if(acc === undefined){
      acc = arr[0]; // acc 초기값이 없을 때
    }else {
      acc = iteratee(acc, ele, idx, arr); // 누적 값이 쌓임
    }
  })
